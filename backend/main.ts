...
async function handleDeleteSite(domain: string) {
  ...
  await Deno.remove(getMetaPath(domain)).catch(() => {});
  await Deno.remove(getRepoHistoryPath(domain)).catch(() => {});
  await Deno.remove(siteDir, { recursive: true }).catch(() => {});
...
async function handler(req: Request, info: Deno.ServeHandlerInfo): Promise<Response> {
  ...
  if (path === '/auth' && req.method === 'GET') {
    if (!API_KEY) return json({ secured: false });
    return req.headers.get('X-Api-Key') === API_KEY
      ? json({ secured: true, valid: true })
      : json({ secured: true, valid: false }, 401);
  }
