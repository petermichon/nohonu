...
import { VALID_USERNAME } from '../paths.ts';
...
export async function extractRegisterParams(req: ExpressReq): Promise<RegisterParams | undefined> {
  ...
  if (!VALID_USERNAME.test(username)) return undefined;
  return { password: body.password, username, userAgent: req.get('User-Agent') || undefined };
}
