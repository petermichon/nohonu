...
export async function downloadVersion(
  ...
  if (!(await fileExists(versionPath(user, siteId, index)))) return { ok: true, value: null };
  const data = await fs.readFile(versionPath(user, siteId, index));
  return { ok: true, value: { data, filename: `${siteId}-${index}.zip` } };
}
