...
interface AddDomainModalProps {
  ...
  sites: Site[];
  onClose: () => void;
  onAdd: (siteDomain: string, customDomain: string) => Promise<void>;
}
...
export function AddDomainModal({ isOpen, sites, onClose, onAdd }: AddDomainModalProps) {
  ...
  const canSubmit = !!trimmed && !!siteDomain && !isInvalid && !isSubmitting;
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await onAdd(siteDomain, trimmed);
      setCustomDomain('');
      onClose();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add custom domain" size="sm">
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="siteDomain" className="text-sm text-zinc-600 dark:text-zinc-400 mb-1.5 block">
            Site
          </label>
          <select
            id="siteDomain"
