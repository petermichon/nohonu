...
export function SiteCard({ site, onToggle, loading }: SiteCardProps) {
  ...
  const uptimeBarColor =
    site.uptime === null ? 'bg-stone-200 dark:bg-stone-700' : site.uptime < 90 ? 'bg-red-400' : 'bg-purple-400';
  ...
  return (
    ...
    >
      {/* Accent strip */}
      <div className={`shrink-0 w-1 self-stretch rounded-full ${uptimeBarColor}`} />
          ...
          <div className="flex items-center gap-2">
            <span
              className={`flex items-center gap-1 text-xs font-medium ${
                site.uptime < 90 ? 'text-red-500 dark:text-red-400' : 'text-purple-500 dark:text-purple-400'
              }`}
            >
            ...
            <div className="w-16 h-1.5 bg-stone-100 dark:bg-stone-800 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${uptimeBarColor}`} style={{ width: `${uptimePercent}%` }} />
            </div>
          </div>
        )}
      </div>
