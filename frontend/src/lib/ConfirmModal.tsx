...
const CONFIG: Record<Action, { title: string; message: string; confirm: string; isDanger?: boolean }> = {
  ...
  'delete-version': { title: 'Delete version?', message: 'This version will be permanently removed.', confirm: 'Delete', isDanger: true },
  'activate-version': { title: 'Activate version?', message: 'This will replace the currently live version.', confirm: 'Activate' },
};
...
export function ConfirmModal({ isOpen, onClose, onConfirm, action, domain, loading }: ConfirmModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="sm">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-4 ${isDanger ? 'bg-purple-200 dark:bg-purple-900/30' : 'bg-stone-100 dark:bg-stone-800'}`}>
