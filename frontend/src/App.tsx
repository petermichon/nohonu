...
import { Rocket, Globe, Server, Sun, Moon, Settings, Check, AlertCircle, Loader2 } from 'lucide-react';
...
function SettingsPopover() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!data.secured) {
        ...
        setStatus('valid');
        setTimeout(() => { setStatus('idle'); setOpen(false); }, 800);
      } else {
        setStatus('invalid');
      }
    } catch {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => { setUrl(apiBase); setKey(apiKey); setStatus('idle'); setOpen(o => !o); }}
